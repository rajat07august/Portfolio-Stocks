# Sunita Tools — Company Profile

**Ticker:** SUNITA · BSE-SME: 544001 · **Sector:** Precision Machining + Mould Bases (CNC / Grinding / Aerospace) · **Conviction:** Medium (with SME caveats)
**Date written:** 3-Jul-2026 · **CMP:** ₹741 · **Market Cap:** ₹465 cr · **P/E (trailing):** 73.5x · **ROCE:** 10.1% · **ROE:** 10.8%
**Sources:** Concall H1FY25 (Nov 2024, only concall available) + Investor deck Sep 2025 · Screener SME view · ValuePickr Sunita Tools thread (last 18 months) · BSE filings

> **Note on sourcing:** Sunita Tools is a very small SME with limited concall history. Only ONE full concall transcript (H1FY25) is available. Financial data extrapolated from investor deck + BSE filings + Screener. Confidence in numbers is moderate; management-tone reading is based on very limited data points. **This profile carries more uncertainty than others in the watchlist.**

<!-- SINCE-BASELINE:START -->
**📍 Live market check — 23-Sep-2026 (Screener):** CMP **₹ 1,201** · market cap ₹ 754 Cr. · P/E 119x · ROCE 10.1 % · ROE 10.8 % · 52-wk ₹ 1,222 / 553 · **vs ₹741 at baseline (+62.1%)**

<details class="upd" id="since-baseline">
<summary><span class="upd-date">🆕 Changes since baseline · 3-Jul-2026 → 23-Sep-2026</span> <span class="upd-sum">🆕 Nothing filed; ₹576cr order book vs ~₹47cr revenue from undisclosed counterparties still stands · (earlier in window: ₹576cr order book (₹552cr defence) against ₹32cr of four-month sales, all from undisclosed counterparties; three defence MoUs; unnamed 90% acquisition; equity raise approved; stock +44%)</span></summary>
<div class="upd-body">

**⚪ 19 → 23-Sep-2026 — Nothing material filed**
Swept **BSE** (544001 — BSE-only): **zero announcements.** The 19-Sep flag stands: **a ₹576cr order book against ~₹47cr of FY26 revenue, all from undisclosed counterparties.**
> **Why it matters:** no change to the thesis, the numbers or the open questions in this profile. Recorded so that the gap in the record is a *checked* gap rather than an unchecked one (§0A rule 6).

*Sweep: NSE + BSE announcements 19-Sep → 23-Sep-2026; market data Screener 23-Sep-2026. BSE's JSON announcements API is currently Akamai-blocked from this machine — the BSE leg was run through Chrome.*

**Net read:** **HELD at SPECULATIVE. This profile was written on 3-Jul from one concall and a deck; nine weeks of company disclosures have since added claims that are large relative to the company and thin on verifiable detail.** The consolidated order book is **₹576cr as of 31-Jul, of which ₹552cr is defence, against FY26 revenue of ~₹47cr** *(Screener half-years)*. **Not one of the following is disclosed: the customer for the 155mm shell agreement, its value, the size of the 'advance tranches', the identity of the Indian OEM in the MoU, or the name and price of the 90% acquisition.** The Board also approved raising equity on 14-Aug, and the stock is **+44% to ₹1,066 at 106x**. Nothing here is shown to be false; it is unverified, and the pattern is the one a small-cap SME investor should hold to a higher evidence bar. **New Under-Watch trigger: disclosure of the shell customer and agreement value, or an equity raise priced off the run-up.**

**17-Sep-2026 — 🟡 Another tranche of advance payment on the 155mm NATO-spec shell 'Interim Sales Agreement'**  
Press release: Sunita Tools has received **another tranche of advance payment** against an Interim Sales Agreement for **NATO-spec and standard 155mm M107 empty shells**; the Chairman says 'we are production ready'. The first tranche was announced 20-Jul ('sizeable'; 2nd and 3rd 'expected to follow shortly').
> *Why it matters:* **No amount, customer, quantity or total contract value is stated in either release.** An 'interim' sales agreement with advance payments is a preliminary arrangement, not a final supply contract. The company is a ~₹47cr-revenue mould-base maker moving into artillery-shell manufacture; production readiness at scale is asserted, not evidenced.

**28-Aug-2026 — 🟡 Business update: ₹31.95cr sales in four months; order book ₹576cr; 90% of an unnamed capital-goods company acquired**  
Consolidated net sales **₹31.95cr for the four months to 31-Jul-2026** (Q1FY27: ₹18.64cr vs ₹4.14cr, +350%). **Consolidated order book ₹576cr — defence ₹552cr, legacy ₹14cr, others ₹10cr.** Two CNC turning machines added to the Ahmedabad subsidiary; **90% of an unnamed 'capital goods manufacturing company' acquired.**
> *Why it matters:* **Defence is 96% of the book and roughly 12x FY26 revenue.** The legacy business the profile analysed has a ₹14cr order book. The acquisition's name, price and financials are not disclosed. The growth the company cites (+350%) is off a base of ₹4cr in Q1FY26.

**25-Aug-2026 (MoU signed 30-Jul) — 🟡 Framework MoU with an undisclosed '135-year-old' Indian defence OEM for 155mm shell tenders**  
The counterparty is not named ('contractual confidentiality'). Sunita would act as **manufacturing partner** while the OEM bids as prime; the filing states the MoU **'does not create a legal consortium … nor constitute an order or tender award or specify any committed quantity, contract value, assured revenue'.**
> *Why it matters:* The company's own wording is the correct reading: a framework, not business. Sunita offers to disclose the counterparty to BSE or SEBI on request.

**30-Jul-2026 — MoU with a Polish defence manufacturer (ISR drones, loitering and smart ammunition)**  
Intent to collaborate on research, design, development, manufacturing, marketing and sale of drones and loitering ammunition.
> *Why it matters:* Non-binding, counterparty unnamed.

**14-Aug-2026 — ⚠️ Board approves raising equity capital**  
Board outcome: 'raising of equity capital through permissible modes, subject to approval of shareholders', with the draft board report and notice of the 39th AGM.
> *Why it matters:* The mode, size and price are not specified. **A raise timed after a 44% run-up on undisclosed-counterparty announcements is the specific risk to watch;** compare Oriana, OBSC and Wheels, where issues were priced by SEBI formula off lower averages.

**3-Jul → 18-Sep-2026 — Price ₹741 → ₹1,066 (+44%); market cap ₹670cr**  
Screener P/E 106x, ROCE 10.1%, ROE 10.8%; promoter 62.75% (Mar-2026; was 72.98% in Mar-2024); 52-week range ₹553–1,150.
> *Why it matters:* The multiple has moved from 74x to 106x on a profile that was already flagged for thin disclosure.

**Routine filings, not material:** Investor presentation (July), book closure and AGM notices, trading-window closures, newspaper notices.

**Where this was blended into the profile:** Dropdown only: the profile is in the original pre-methodology format (single concall, deck) and needs a full rewrite before any of the above can be blended into its sections.

*Sweep: BSE + NSE announcements from 3-Jul-2026 to 19-Sep-2026; market data Screener 18-Sep-2026 close. Items above are facts from the filings named; anything marked as inference is mine, not the company's.*

</div>
</details>
<!-- SINCE-BASELINE:END -->

<!-- FACTORS:START -->
<details class="upd" id="factor-check">
<summary><span class="upd-date">📈 Input &amp; factor check · 19-Sep-2026</span> <span class="upd-sum">What the prices this business buys and sells have done lately, and what that means for margins.</span></summary>
<div class="upd-body">

**Read (19-Sep-2026, mine, not management's):** Flat steel is +10.0% over three months and at a 52-week high. History shows steel is billed through (revenue fell when steel fell 45%), so this should lift revenue more than it hurts margin.

**How cost moves reach profit:** Steel is billed through: when steel fell 45%, H1FY25 revenue fell even as volume rose 38–40% (profile §4).

**Sensitivity:** raw materials were **64.0% of sales in FY26** (FY25: 52.0%, Screener). A 10% move across the whole raw-material bill is worth about **640 bps of margin before any pass-through**. The tracked inputs averaged **+10.0% over three months**, which would be roughly **640 bps of pressure** if the whole bill moved the same way. *That is an upper bound: it is a simple average (weights are not disclosed) and it ignores pass-through.*

| Factor | Role | 1M | 3M | 1Y | vs 52w high | Effect | Source · as of |
|---|---|---|---|---|---|---|---|
| Steel Flat | Input cost | +9.3% | +10.0% | +29.5% | +0.0% | 🔴 headwind | Tijori (indianpetrochem.com) · 19 Sep, 09:30 am IST |

**Why each factor is here:**

- **Steel Flat**: Steel plate machining and mould bases, bought direct from mills (profile Moat 6).


*Effect is judged on the 3-month move (flat if under 3%); for India macro rows, on the latest YoY growth (tailwind above 5%, headwind below 0). Tijori figures are percentage moves only (no price levels) and come from a single industry source (indianpetrochem.com), so they are indicative until a company's own disclosure confirms them. Snapshot pulled 19-Sep-2026 22:59 by `tools/factors/pull.py`.*

</div>
</details>
<!-- FACTORS:END -->

---

## 1. Business in Plain English

Sunita Tools is a **small-cap SME precision machining company** — CNC machining, grinding, mould base manufacturing, and steel plate machining — with facilities in **Vasai (Maharashtra)**, **Ahmedabad**, and **Coimbatore** (recently acquired). Younger, smaller, and more entrepreneurial than the mid-large auto components players, targeting the precision engineering + aerospace + mould-based industries.

The 30-second business snapshot:
- **HQ Vasai, Maharashtra** — plus recently acquired Ahmedabad (Sunita Leo) + Coimbatore (Sunita Imperial) units
- **First-time promoter:** *Mr. Satish Kumar Pandey* — Managing Director AND CFO (both roles on one person = classic SME structure)
- **Small revenue base** (INR ~50-70 cr range based on H1FY25 data)
- **H1FY25 volume growth 38-40%**; revenue slightly lower due to 45% steel price drop
- **Long-term debt-free** (paid off last loan Aug 2024)
- **Recent acquisitions**: Sunita Leo (Ahmedabad), Sunita Imperial (Coimbatore in progress), LOI with Chennai mould base company
- **Aerospace orders** at Vasai factory 2
- **Distribution partnerships**: Saudi + UAE (early stage)
- **Screener listing: Sunita Tools Ltd — CMP ₹741, MCap INR 465 cr, P/E 73.5x**

**Big picture in one sentence:** Sunita Tools is a **founder-driven SME precision machining player** on an acquisition-led growth strategy — Ahmedabad + Coimbatore + Chennai expansions — but with meaningful promoter dilution (72.98% → 62.75% over 2 years) and only 1,733 shareholders (extreme illiquidity). Priced at 73.5x P/E on modest returns (ROCE 10.1%, ROE 10.8%) — **the most speculative name in the auto components sector**.

---

## 2. Why This Industry Exists

Precision machining rides same forces as bigger peers but at smaller scale:
1. Domestic auto capex cycle
2. Aerospace India + defence indigenisation
3. Mould base + tool room outsourcing
4. Export potential (Saudi/UAE partnerships)

**Where Sunita sits.** Very small, sub-scale by industry standards. Growing via acquisitions.

---

## 3. What Makes This Company Different

**Moat 1 — First-time promoter learning curve.** Discipline growing.
**Moat 2 — Acquisition-led growth strategy.** Sunita Leo + Coimbatore + Chennai.
**Moat 3 — Aerospace order pipeline (Vasai factory 2).**
**Moat 4 — Long-term debt-free.** Balance sheet discipline.
**Moat 5 — Coimbatore expansion into "world's largest auto cluster."**
**Moat 6 — Direct steel mill relationships** (Arcelor Mittal, Jindal — avoiding distributor 5-10% margins).
**Moat 7 — Vasai land acquired for future expansion.**

**Where the moat is weaker.**
- Very small size — customer concentration risk.
- Founder-CFO combined role.
- SME liquidity — 1,733 shareholders (extreme illiquidity).
- Limited concall history (only 1 available).
- Screener valuation data limited.
- 10.23% promoter dilution over 2 years.
- Multiple small acquisitions = execution risk.

---

## 3.5. Management Track Record & Promoter Background

**Managing Director & CFO:** *Mr. Satish Kumar Pandey* — first-time promoter (acknowledged on H1FY25 concall). Combined MD + CFO role — SME structure.

**Director (Sunita Leo Ahmedabad):** *Mr. Prashan Gupta* — professional director for Ahmedabad acquisition.

**Promoter — Pandey family.**

**Promoter holding (Mar 2026): 62.75% — down significantly from 72.98% (Oct 2023):**

| Period | Promoter % |
|---|---|
| Oct 2023 | 72.98% |
| Mar 2024 | 72.98% |
| Mar 2025 | 67.18% (5.80% dilution) |
| Oct 2025 | 67.71% |
| Mar 2026 | 62.75% (further 4.96%) |

**~10.23% cumulative dilution over 2.5 years.** Significant — funding acquisitions + working capital.

**Other listed entities:** None.

**Delivery track record (limited data):**

| Guidance / Milestone | When | Actual delivered | Verdict |
|---|---|---|---|
| Volume growth 38-40% | H1FY25 | Delivered | ✅ Delivered |
| Long-term debt payoff | H1FY25 | Delivered Aug 2024 | ✅ Delivered |
| Sunita Leo (Ahmedabad) acquisition | H1FY25 | Delivered | ✅ Delivered |
| Sunita Imperial (Coimbatore) acquisition | H1FY25 | In progress | 🕐 In progress |
| Chennai mould base LOI conversion | H1FY25 | LOI signed | 🕐 In progress |
| Aerospace orders (Vasai factory 2) | H1FY25 | Orders received | ✅ Delivered |
| Gulf distribution (Saudi/UAE) | H1FY25 | Partnerships setup | 🕐 In progress |

**Verdict on management:** Limited data (only 1 concall). Acquisition-led growth story with meaningful dilution. Bet on Pandey's execution — but with SME risk factors clearly present.

---

## 4. Numbers Decoded

**H1FY25 (Nov 2024 concall) — the only detailed data point:**
- Volume growth 38-40% YoY
- Revenue slightly lower due to steel price drop 45%
- Debt paid off Aug 2024
- Preferential shares allotted (dilution)

**Screener data (Mar 2026):**
- Market cap: INR 465 cr
- CMP: INR 741
- P/E: 73.5x
- Book value: INR 112 (P/B 6.6x)
- ROCE: 10.1%
- ROE: 10.8%
- Dividend yield: 0.00%
- Only 1,733 shareholders

---

## 5. Connecting the Dots

**Thread A — Ahmedabad (Sunita Leo) acquisition scaling.**
**Thread B — Coimbatore expansion into auto cluster.**
**Thread C — Chennai mould base acquisition.**
**Thread D — Aerospace orders at Vasai.**
**Thread E — Gulf distribution partnerships.**

---

## 6. Why the Market Is Paying This Multiple

Limited market attention (1,733 shareholders). Priced for aggressive acquisition strategy + expansion narrative.

**Fair value framework (highly uncertain due to limited data):**
- **Base case:** Acquisitions integrate, revenue doubles over 2 years. ~30-50% upside.
- **Bull case:** All expansions succeed + aerospace scale. ~80-100% upside.
- **Bear case:** Execution slippage, further dilution, working capital blow-up. ~30-40% downside.

Weighted return: ~10-15% over 2 years — but with very wide error bars given SME + limited data.

---

## 7. What Could Prove Us Wrong

**Risk 1 — Small size + customer concentration.**
**Risk 2 — Combined MD + CFO role (SME governance).**
**Risk 3 — Extreme illiquidity (1,733 shareholders).**
**Risk 4 — Continued promoter dilution.**
**Risk 5 — Multi-acquisition integration failures.**
**Risk 6 — Working capital pressure with fast expansion.**
**Risk 7 — Very limited concall disclosure history.**
**Risk 8 — Aerospace order execution risk.**
**Risk 9 — Gulf distribution slow ramp.**
**Risk 10 — First-time promoter learning curve.**

---

## 8. 30-Second Memory Hook

> **Sunita Tools is a promising Ranji Trophy player — talented, showing potential, but not yet tested in international cricket.** Small SME precision machining with acquisitions (Ahmedabad + Coimbatore + Chennai LOI + aerospace at Vasai + Gulf distribution). H1FY25 volume +38-40%, debt-free. First-time promoter Satish Pandey serving as both MD and CFO.
>
> The captain (Pandey) is aggressive on acquisitions — but promoter holding dropped 10.23% in 2.5 years, only 1,733 shareholders (extreme illiquidity), and only 1 detailed concall transcript exists.
>
> **What you're buying:** a SME precision machining growth story at 73.5x P/E with meaningful acquisition-led expansion but limited disclosure history. **What you're risking:** extreme illiquidity, continued dilution, integration failures, small size customer concentration, and limited transparency. Weighted 2-year return highly uncertain: base case ~10-15% with very wide error bars.
>
> **What to watch:** any full quarterly concalls (quarterly cadence begins next FY per management), Sunita Imperial Coimbatore commissioning, Chennai mould base acquisition close, and further preferential issue announcements. Position sizing: **speculative — highest-risk name in the auto components sector. Small position size only.**

---

## Source Appendix

- **Concalls read:** H1FY25 (Nov 2024, only detailed concall available); investor deck Sep 2025.
- **Screener:** Sunita Tools Ltd (BSE: 544001).
- **BSE filings:** Acquisition announcements, preferential issue disclosures.
- **ValuePickr — Sunita Tools thread:** Limited coverage.
- **Peer references:** Very limited direct peer set at this SME scale.

---

*Content is personal investment research and thesis-building for portfolio decisions. Not a buy/sell recommendation. All views subject to revision as new data arrives.*
